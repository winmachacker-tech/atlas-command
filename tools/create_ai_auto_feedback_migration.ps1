<# tools\create_ai_auto_feedback_migration.ps1
   Purpose: Create a fresh, unique migration file that auto-inserts driver_feedback on Delivered/Completed loads,
            then push it with npx supabase db push (no global install needed).
   Usage:
     powershell -ExecutionPolicy Bypass -File ".\tools\create_ai_auto_feedback_migration.ps1" -ProjectPath "C:\Users\mtish\OneDrive\Desktop\atlas-command" -ProjectRef "tnpesnohwbwpmakvyzpn"
#>
...
if (-not (Test-Path $ProjectPath)) { Write-Err "ProjectPath not found: $ProjectPath"; exit 1 }

$migrations = Join-Path $ProjectPath "supabase\migrations"
if (-not (Test-Path $migrations)) {
  Write-Info "Creating migrations folder: $migrations"
  New-Item -ItemType Directory -Path $migrations -Force | Out-Null
}

# Unique 14-digit timestamp (yyyyMMddHHmmss)
function New-Version {
  do {
    $v = (Get-Date).ToString("yyyyMMddHHmmss")
    Start-Sleep -Milliseconds 200
    $probe = Join-Path $migrations "${v}_ai_auto_feedback_on_delivered.sql"
  } while (Test-Path $probe)
  return $v
}

$version = New-Version
$file    = Join-Path $migrations "${version}_ai_auto_feedback_on_delivered.sql"

Write-Info "Creating migration: $file"

$sql = @"
-- ${version}_ai_auto_feedback_on_delivered.sql
-- Purpose: Auto-insert driver_feedback 👍 when a load is delivered/completed.
...
-- Build lane key like "Tulsa, OK → Columbus, OH"
...
-- === TRIGGER: on loads status change, auto-write 👍 feedback ===
...
Write-Info "Wrote migration file."

# Link + push via npx (no global CLI install required)
Push-Location $ProjectPath
try {
  Write-Info "Linking project (if needed)…"
  npx supabase link --project-ref $ProjectRef | Out-Host
} catch {
  Write-Warn "Link may already exist; continuing…"
}
...
Write-Info "Done. Migration applied (or there were no changes). Now mark a load Delivered/Completed and press Train AI."
