    ...
    <div className="p-3 sm:p-6">
      <div className="mb-4">
        ...
        <p className="text-xs sm:text-sm text-white/60">
          Thumbs history, evolving prefs, and acceptance trends.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
        ...
        <div className="rounded-2xl border border-white/10">
          <div className="bg-white/5 px-3 py-2 text-xs font-semibold">
            Drivers
          </div>
...
function GlobalRecent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) {
        console.error("Failed to load global driver feedback:", error);
        setRows([]);
      } else {
  ...
  return (
    <div className="mt-6 rounded-2xl border border-white/10 overflow-hidden">
      <div className="bg-white/5 px-4 py-2 text-xs font-semibold">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-white/10">
                ...
                <Td>
                  {[r.driver_last_name, r.driver_first_name]
                    .filter(Boolean)
                    .join(", ") || "—"}
                </Td>
                <Td>{r.accepted ? "Up" : "Down"}</Td>
                <Td>{r.load_reference || "—"}</Td>
                <Td>
                  {r.origin || "—"} → {r.destination || "—"}
                </Td>
                <Td>{r.equipment_type || "—"}</Td>
                <Td>{r.miles ?? "—"}</Td>
                <Td className="max-w-[320px] truncate" title={r.note || ""}>
                  {r.note || "—"}
                </Td>
              </tr>
            ))}
...
function Th({ children }) {
  return <th className="px-4 py-2 text-white/70">{children}</th>;
}
