...
export default function DispatchAI() {
  ...
  async function fetchData() {
    ...
    setActionRecId(null);
    try {
      // Include load_number so we can show it
  ...
  async function runCommand(text) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Even on error, the function may log a recommendation — surface it
      ...
      // Success
      setActionMsg(
        dryRun ? `Dry run: ${data?.message || "ok"}` : data?.message || "ok"
      );
      if (data?.recommendation?.id) setActionRecId(data.recommendation.id);
      if (!dryRun) fetchData();
    } catch (e) {
  ...
  // Always pass the UUID to the function to satisfy its parser
  ...
  function handleAssigned(payload) {
    ...
    if (payload?.ok) {
      ...
      if (payload?.recommendation?.id)
        setActionRecId(payload.recommendation.id);
      fetchData();
    }
  }
  ...
  const statusNote = useMemo(() => {
    if (!envs.functionsUrl)
      return "Missing FUNCTIONS URL — set VITE_SUPABASE_FUNCTIONS_URL or VITE_SUPABASE_URL.";
    if (!envs.anonPresent)
      return "Missing ANON KEY — set VITE_SUPABASE_ANON_KEY.";
        ...
        <div className="mt-8">
          ...
          {loading && (
            <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-4 text-sm text-zinc-400">
              Loading available loads and drivers...
            </div>
          )}

          {!loading && suggestions.length === 0 && (
            <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-4 text-sm text-zinc-400">
              No quick suggestions right now. Make sure you have{" "}
              <b>AVAILABLE</b> loads and <b>ACTIVE</b> drivers.
            </div>
          )}
