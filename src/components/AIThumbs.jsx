...
// Usage example:
//   <AIThumbs driverId={driver.id} laneKey={laneKey} onAfterChange={refetch} />
...
export default function AIThumbs({
  ...
  const sizes = { sm: "h-7 px-2 text-xs gap-1", md: "h-8 px-3 text-sm gap-1.5", lg: "h-10 px-4 text-base gap-2" };
  ...
  async function handle(kind) {
    if (!driverId || !laneKey) { setErrMsg("Missing driver or lane"); return; }
    if (throttle(600)) return;

    setBusy(true); setOkMsg(""); setErrMsg("");
    const label = kind === "up" ? 1 : -1;

    try {
      const { error } = await supabase.from("ai_training_examples").insert([
        { driver_id: driverId, lane_key: laneKey, label, source: "thumb" },
      ]);
      ...
      if (error) {
        ...
        const dup = code === "23505" || msg.includes("duplicate key value") || msg.includes("uniq_ai_examples_guard");
        if (!dup) throw error;
      }
      ...
      console.error("AIThumbs insert failed:", e);
  ...
  const upBtn = "bg-emerald-600/15 border-emerald-600/30 hover:bg-emerald-600/25 text-emerald-200";
  const downBtn = "bg-rose-600/15 border-rose-600/30 hover:bg-rose-600/25 text-rose-200";
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      >
        {busy ? <Loader2 size={iconSizes[size]} className="animate-spin" /> : <ThumbsUp size={iconSizes[size]} />}
        <span>Up</span>
      </button>
      ...
      >
        {busy ? <Loader2 size={iconSizes[size]} className="animate-spin" /> : <ThumbsDown size={iconSizes[size]} />}
        <span>Down</span>
      </button>
