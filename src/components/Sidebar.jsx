...
import { LayoutDashboard, ClipboardList, Route, Truck, Shield } from "lucide-react"; // ← added Shield
...
const nav = [
  ...
  { to: "/admin/audit", label: "Audit Log", icon: Shield }, // ← NEW
];
...
export default function Sidebar() {
  return (
    <aside className="hidden md:flex md:flex-col w-64 shrink-0 border-r border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-950">
      <div className="h-14 flex items-center px-4 font-semibold tracking-wide">Atlas Command</div>
        ...
        {nav.map(({ to, label, icon: Icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              ...
              (isActive ? "bg-black text-white" : "hover:bg-gray-100 dark:hover:bg-neutral-900")
            }
            end={to === "/"}
          >
            <Icon size={18} />
