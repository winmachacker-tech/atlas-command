...
async function ensureUserRow(profile = {}) {
  try {
    ...
    // Upsert minimal profile into public.users
    const payload = {
      id: user.id,
...
export default function PhoneSignup() {
  ...
  const [phase, setPhase] = useState<"enter" | "verify" | "done">("enter");
  ...
  async function requestOtp(e) {
    ...
    if (!/^\+?[1-9]\d{6,14}$/.test(normalized)) {
      setMsg({ type: "bad", text: "Enter a valid E.164 phone number (e.g. +14155551234)." });
      return;
    }
    ...
    try {
      ...
      // Start phone OTP sign-in. Attach initial metadata.
  ...
  async function verifyOtp(e) {
    ...
    try {
      ...
      setPhase("done");
      // optional: route to onboarding if you want to capture more fields
    ...
    <div className="mx-auto max-w-xl px-6 py-10">
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-xl">
        <div className="mb-6 flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-zinc-100">Sign up with Phone</h1>
        </div>
          ...
          <form onSubmit={requestOtp} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="mt-1 text-xs text-zinc-500">
                This is saved to your profile on first sign-in.
              </p>
            </div>
